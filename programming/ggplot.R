...
pl <- haven::read_dta(file.path(path_soep_data, "pl.dta"))
bioedu <- readRDS("raw/bioedu.rds")
regionl <- readRDS("raw/regionl.rds")
...
# Filter the missings in job satisfaction
merge_data_na <- merge_data[merge_data$plh0173 >= 0 & merge_data$pla0009_h>0, ]
 
# Estimate the mean of employment satisfaction
mean_data <- aggregate(plh0173 ~ syear + pla0009_h, data = merge_data_na, FUN = mean, na.rm = TRUE)
...
merge_data_na$graduates <- NA
merge_data_na$graduates[merge_data$bex8cert==-2] <- "No Tertiary Degree"
merge_data_na$graduates[merge_data$bex8cert==1] <- "Applied University"
merge_data_na$graduates[merge_data$bex8cert==2] <- "University"
merge_data_na$graduates[merge_data$bex8cert==3] <- "Doctorate"
merge_data_na$graduates[merge_data$bex8cert==4] <- "Vocational College"
merge_data_na$graduates[merge_data$bex8cert==5] <- "Other"
table(merge_data_na$graduates, useNA = "always")


mean_grad <- aggregate(plh0173~syear+graduates, 
                       data=merge_data_na, 
                       FUN=mean, 
                       na.rm=TRUE)
...
## Save the data
saveRDS(merge_reg, "data/merge_reg.rds")
